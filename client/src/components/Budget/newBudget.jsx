...
const NewBudget = () => {
  ...
  return (
    <AnimatePresence>
      {snap.isNewBudget && (
        <motion.div>
          <div className="absolute z-20 top-0 w-full h-full bg-black bg-opacity-40" />
          <div className="absolute  z-20 left-0 right-0 bottom-0 bg-[#F6F3F3] rounded-tl-[30px] rounded-tr-[30px] pt-[39px] px-[22px] pb-10">
            <div
              className="absolute top-2 cursor-pointer right-4"
              onClick={() => (state.isNewBudget = false)}
            >
              <img
                src={close}
                alt="Budget Image"
